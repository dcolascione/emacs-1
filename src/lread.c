  ...
  if (!SYMBOLP (tem))
    {
	 ...
	 We could just use make_string here and live with the extra copy.  */
      ...
      tem = intern_driver (make_c_string (str, len), obarray, tem);
    }
  return tem;
}
...
#ifdef HAVE_MODULES
#ifdef MODULES_SECONDARY_SUFFIX
  Vload_suffixes = list4 (build_c_string (".elc"),
			  build_c_string (".el"),
			  build_c_string (MODULES_SUFFIX),
                          build_c_string (MODULES_SECONDARY_SUFFIX));
#else
  Vload_suffixes = list3 (build_c_string (".elc"),
			  build_c_string (".el"),
			  build_c_string (MODULES_SUFFIX));
#endif
#else
